...
const CarInBrand = () => {
    ...
    const getCar = async () => {
        try {
            // const { data } = await axios.get(`${process.env.REACT_APP_API}/api/brand/getBrandBtId-brand/${params.slug}`);
            const { data } = await axios.get(`/api/brand/getBrandBtId-brand/${params.slug}`);
            setBrand(data.brand);
            setLoading(false);
        } catch (err) {
            console.log(err);

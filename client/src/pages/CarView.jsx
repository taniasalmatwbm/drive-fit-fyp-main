...
const CarView = () => {
    ...
    const getRelatedCar = async (cid, bid) => {
        try {
            // const { data } = await axios.get(`${process.env.REACT_APP_API}/api/car/related-car/${cid}/${bid}`)
             const { data } = await axios.get(`/api/car/related-car/${cid}/${bid}`)
    ...
    const galleryImages = Array.isArray(car.productPictures)
  ? car.productPictures.map(picture => ({
      original: picture,
      thumbnail: picture
    }))
  : [];
                ...
                <div className="row">
                        ...
                        <div className='centerMob'>
                            <Link to={`/brand/${car.brand.name}`}>
                                <img
                                    ...
                                    src={car.brand?.brandPictures || "/default-brand.jpg"}
                                    className="img-fluid"
                    ...
                    <div className="container mt-5">
                        <div className="row">
                            ...
                            {relatedCar.length > 0 ? (
                                relatedCar.map((p) => (
                                    <div key={p._id} className="col-md-12 col-lg-3 mb-3 mb-lg-0 my-3">
                                        <div className="card">
                                            <div className="d-flex justify-content-between p-3">
                                                ...
                                                <Link to={`/brand/${p.brand.slug}`}
                                                    ...
                                                    <p className="text-white mb-0 small">
                                                        <img src={p.brand?.brandPictures || "/default-brand.jpg"} alt={p.brand.name} style={{ maxWidth: '100%', maxHeight: '150px', objectFit: 'contain' }} />
                                                    </p>
                                                </Link>
                                            </div>
                                            <Link to={`/car/${p.slug}`} className='text-center'>
                                                <img src={p.productPictures?.[0] || "/default-car.jpg"} alt={p.name} style={{ maxWidth: '100%', maxHeight: '120px', objectFit: 'contain' }} className='border rounded' />
                                            </Link>
